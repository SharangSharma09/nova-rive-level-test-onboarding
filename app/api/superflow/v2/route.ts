...
export async function POST(request: Request) {
  ...
  console.log("[v2] ── POST received ──────────────────────────");

  let formData: FormData;
  try {
    formData = await request.formData();
  } catch (e) {
    console.error("[v2] ✗ failed to parse formData", e);
    return Response.json({ error: "Could not process audio. Please try again." }, { status: 400 });
  }

  const audio = formData.get("audio");
  if (!audio || !(audio instanceof Blob)) {
    console.error("[v2] ✗ missing audio");
  ...
  console.log("[v2] audio received —", Math.round(arrayBuffer.byteLength / 1024), "KB,", mimeType);

  // ── Step 1: STT ────────────────────────────────────────────────────────────
  let transcription: string;
  try {
    transcription = await transcribeAudio(base64, mimeType, "v2");
    if (!transcription) {
      console.warn("[v2] ✗ audio unclear/empty");
      return Response.json({ error: "I couldn't catch that. Please try again." }, { status: 422 });
    }
  } catch (e) {
    console.error("[v2] ✗ STT error:", e);
  ...
  try {
    ...
    console.log("[v2] classifier raw:", classifyText);

    const parsed = ClassifierSchema.safeParse(JSON.parse(classifyText));
    if (parsed.success) {
      if (parsed.data.confidence === "low") {
        intent = "draft";
        console.log("[v2] low confidence → defaulting to draft");
      } else {
        intent = parsed.data.intent;
      }
      detectedLanguage = parsed.data.detectedLanguage || "Hindi";
      console.log(`[v2] classified as: ${intent} | language: ${detectedLanguage} | confidence: ${parsed.data.confidence}`);
    } else {
      console.warn("[v2] classifier parse failed, defaulting to draft:", parsed.error.flatten());
    }
  } catch (e) {
    console.error("[v2] ✗ classifier error, defaulting to draft:", e);
  }
  ...
  try {
    if (intent === "draft") {
      ...
      console.log("[v2:draft] LLM responded in", Date.now() - start, "ms");
      ...
      console.log("[v2:translate] done in", Date.now() - start, "ms");
      ...
      const text = stripFences(result.response.text().trim());
      console.log("[v2:grammar] done in", Date.now() - start, "ms");
      ...
      console.log("[v2:meaning] done in", Date.now() - start, "ms");
    ...
    console.error("[v2] ✗ LLM error after", Date.now() - start, "ms:", e);
