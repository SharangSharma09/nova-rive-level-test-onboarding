...
type V5Mode = "localize" | "doubt" | "upload";
type V5DoubtIntent = "translate" | "meaning" | "grammar" | "doubt";
type V5State = "closed" | "mode_select" | "recording" | "upload_form" | "analyzing" | "result" | "error";
...
const LANG_COPY = {
  Tamil: {
    ...
    option3: "Photo / PDF upload பண்ணி explanation பெறுங்க",
  },
  Hindi: {
    ...
    option3: "Photo / PDF upload करें, explanation पाएं",
  },
};

export default function FloatingWidgetV5Upload({
  lang = "Tamil",
  apiEndpoint = "/api/superflow/v5",
  uploadEndpoint = "/api/superflow/v5-upload",
}: FloatingWidgetV5Props = {}) {
  ...
  const onFileSelected = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.value = "";
    if (!file) return;
    console.log("[v5-upload] file selected —", file.name, file.type, file.size);
    const ok = file.type.startsWith("image/") || file.type === "application/pdf";
    if (!ok) {
      setErrorMsg("Please pick a photo (JPG/PNG) or a PDF.");
      setState("error");
      return;
    }
    if (file.size > 15 * 1024 * 1024) {
      setErrorMsg("That file is too big. Please keep it under 15 MB.");
      setState("error");
      return;
    }
    setUploadFile(file);
  ...
  const submitUpload = useCallback(async () => {
    ...
    formData.append("question", uploadQuestion.trim());
    console.log(`[v5-upload] POST ${uploadEndpoint} — ${uploadFile.name}, question:`, uploadQuestion.trim() || "(none)");
    ...
    try {
      ...
      console.log("[v5-upload] response:", data);
      setResult(data);
      setActiveDraftText(null);
      setState("result");
    } catch (e) {
      console.error("[v5-upload] API error:", e);
      setErrorMsg("Something went wrong. Please try again.");
      setState("error");
    }
  }, [uploadFile, uploadQuestion, uploadEndpoint]);
  ...
  return (
    <>
      ...
      {/* Popup panel */}
      {(state === "mode_select" || state === "upload_form" || state === "analyzing" || state === "result" || state === "error") && (
        <div
        ...
        >
          ...
          {state === "mode_select" && (
            <div className="flex flex-col gap-3 rounded-3xl p-4" style={{ ...PANEL_STYLE, width: "min(296px, 80vw)" }}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm leading-snug" style={{ color: "#fff" }}>{copy.option3}</p>
                </div>
              </button>
            </div>
          )}

          {/* Upload form — pick file, optional question, then Ask */}
          {state === "upload_form" && uploadFile && (
            <div className="flex flex-col gap-3 rounded-3xl p-4" style={{ ...PANEL_STYLE, width: "min(320px, 80vw)" }}>
              ...
              {/* File chip */}
              <div className="flex items-center gap-3 rounded-2xl p-3" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
                {uploadFile.type.startsWith("image/") ? (
                  ...
                  <div className="w-12 h-12 rounded-lg flex items-center justify-center text-2xl flex-shrink-0" style={{ background: "rgba(255,255,255,0.08)" }}>
                    📄
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate" style={{ color: "#fff" }}>{uploadFile.name}</p>
                  <p className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>
                    {uploadFile.type === "application/pdf" ? "PDF" : "Photo"} · {Math.round(uploadFile.size / 1024)} KB
                  </p>
                </div>
                <button
                  onClick={openFilePicker}
                  className="text-xs font-semibold px-2.5 py-1.5 rounded-lg transition-all active:scale-95 flex-shrink-0"
                  style={{ color: "#a78bfa", background: "rgba(109,40,217,0.25)" }}
                >
                  Change
                </button>
              </div>
              ...
              <textarea
                ...
                rows={2}
                placeholder="Ask anything about it… (optional)"
                className="w-full resize-none text-sm rounded-2xl p-3 focus:outline-none leading-relaxed"
              ...
              >
                {uploadQuestion.trim() ? "Ask Nova" : "Explain this"}
              </button>
            </div>
          )}
          ...
          {state === "result" && result && (
            ...
            >
              ...
              {/* ── Upload result (photo / PDF explanation) ── */}
              {result.mode === "upload" && result.answer && (
                <>
                  {/* Header — back + speaker */}
                  <div className="flex items-center justify-between">
                    <button
                  ...
                  <ScrollFade>
                    ...
                    {/* Answer */}
                    <div>
                      <p className="text-xs font-bold uppercase tracking-widest mb-1.5" style={{ color: "#00d9a0", letterSpacing: "0.1em" }}>
                        Answer
                      </p>
                      <p className="text-sm leading-relaxed break-words whitespace-pre-wrap" style={{ color: "rgba(255,255,255,0.9)" }}>
                        {result.answer}
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleCopyAnswer(result.answer!)}
                      className="flex-none px-5 py-3 rounded-2xl font-semibold text-sm transition-all active:scale-95 flex items-center justify-center gap-1.5"
                      style={{ background: "#6d28d9", color: "#fff" }}
                    >
                      {answerCopied ? <><CheckIcon /><span>Copied</span></> : <><CopyIcon /><span>Copy</span></>}
      ...
      {/* Hidden file input for photo / PDF upload */}
      <input
        ref={fileInputRef}
        type="file"
