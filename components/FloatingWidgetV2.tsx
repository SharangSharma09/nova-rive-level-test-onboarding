...
type V2State = "closed" | "recording" | "analyzing" | "result" | "error";
...
export default function FloatingWidgetV2() {
  const [state, setState] = useState<V2State>("closed");
  ...
  const startRecording = useCallback(async () => {
    console.log("[v2] starting recording");
    try {
  ...
  const stopAndAnalyze = useCallback(async () => {
    ...
    console.log("[v2] POST /api/superflow/v2 — blob size:", blob.size);

    try {
      const res = await fetch("/api/superflow/v2", { method: "POST", body: formData });
      ...
      console.log("[v2] STT transcription:", data.transcription);
      console.log(`[v2] classified as: ${data.feature} | language: ${data.detectedLanguage}`);
